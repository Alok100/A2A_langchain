...
This script creates and manages a PostgreSQL database for an attendance management system.
...
def create_database():
    ...
    try:
        ...
        if not exists:
            # Use proper escaping for database name
...
def create_database_and_tables():
    ...
    try:
        # Drop attendance table first if it exists (to avoid FK constraint issues)
        ...
        if not employees_exists:
            ...
            # Verify the employees table has the employee_id column
            cursor.execute("""
                SELECT EXISTS (
            ...
            """)
            has_employee_id = cursor.fetchone()[0]
            if not has_employee_id:
                # Drop and recreate employees table if structure is wrong
                ...
                """)
                print("[OK] Employees table recreated with correct structure")
            else:
                print("[INFO] Employees table already exists")
        
        # Commit employees table before creating foreign key
        conn.commit()
        
        # Now create attendance table with foreign key constraint
        cursor.execute("""
...
def insert_dummy_employees():
    ...
    try:
        ...
        employees = [
            ...
            (9, "James Garcia", "HR", 1, date(2020, 4, 1), 1),
            (10, "Amanda Lee", "HR", 0, date(2023, 5, 20), 0),  # License expired
        ]
        ...
        for i in range(5):
            ...
            for emp_id_tuple in valid_employees[:5]:  # First 5 valid employees
                ...
                attendance_records.append((
                    emp_id,
                    check_date,  # PostgreSQL supports native date objects
                    check_in,    # PostgreSQL supports native time objects
                    check_out    # PostgreSQL supports native time objects
                ))
...
def add_new_employee(
    ...
    try:
        ...
        cursor.execute("""
            ...
            (employee_id, employee_name, department, license_valid, date_of_joining, leave_available)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (employee_id, employee_name, department, license_valid_int, date_of_joining, leave_available))
    ...
    try:
        ...
        if existing:
            ...
            if update_fields:
                ...
                # Use psycopg2's sql module for safe SQL construction
                query = f"""
                    UPDATE attendance 
...
def delete_all_records():
    ...
    try:
        ...
        print(f"[ERROR] Error deleting records: {e}")
        conn.rollback()
        raise
    finally:
        cursor.close()
