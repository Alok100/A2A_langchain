...
- **Online flow:** `hr_decision_graph.png` or `architecture_online.png` (align node names with this spec).
- **Offline flow:** `policy_ingestion_graph.png` or `architecture_offline.png`.
- **High-level:** `architecture.png` (User → CLI → LangGraph → LLM + MCP + Policies + LangSmith).
- All diagrams live in the **architecture/** folder. Run `python make_architecture_diagram.py` from that folder to regenerate PNGs.
