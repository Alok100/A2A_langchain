...
from typing import Dict, Tuple, List, Optional  # noqa: F401 Optional for main(out_dir)
...
class Edge:
    ...
    curve: float = 0.0     # arc curvature; 0 = straight
...
def _connect_points(
    ...
    """
    Pick sensible start/end points on the box boundary depending on direction.
    Returns (start_xy, end_xy, label_xy).
    """
    x1, y1 = src_xy
    x2, y2 = dst_xy
    dx, dy = x2 - x1, y2 - y1

    # Prefer connecting on the dominant axis to reduce overlap.
    if abs(dx) > abs(dy):
...
def draw_flowchart(nodes, edges, positions, title, outpath, figsize=(18,10), dpi=220):
    ...
    for e in edges:
        if isinstance(e, tuple):
            # Back-compat with (src, dst, label, style)
...
def draw_architecture(outpath: str = "architecture.png"):
    """
    High-level architecture image (not the detailed node graph).
    Matches the current Project3 design:
      CLI -> LangGraph (graph.py) -> LLM (Ollama) and MCP (Azure Postgres) + Policy docs + LangSmith
    """
    nodes = {
        "user": "User",
        "cli": "CLI (cli_chat.py)\nlogin + chat loop",
    ...
    # Manual layout: left-to-right with supporting services around.
    pos = {
        "user": (0.10, 0.70),
        "cli": (0.28, 0.70),
        "langgraph": (0.50, 0.70),
    ...
    # Bigger figure and boxes for architecture
    draw_flowchart(
        nodes,
        edges,
        pos,
        title="Project3 Architecture (High-Level)",
        outpath=outpath,
        figsize=(18, 9),
        dpi=240,
    )
...
def main(out_dir: Optional[Path] = None):
    """Generate all architecture PNGs. If out_dir is None, writes to current working directory."""
    out_dir = out_dir or Path.cwd()
    out_dir = Path(out_dir)
    # ----------------------------
    # 1) ONLINE DECISION GRAPH
    # ----------------------------
    online_nodes = {
        "start": "__start__",
    ...
    online_positions = {
        "start": (0.5, 0.95),
        "load_session": (0.5, 0.88),
        "validate_user": (0.5, 0.80),
        "load_ltm": (0.5, 0.72),
        "classify": (0.5, 0.64),

        "simple_read": (0.18, 0.52),
        "rag": (0.50, 0.52),
        "draft_action": (0.82, 0.52),

        "policy_reason": (0.50, 0.42),

        "data_reason": (0.72, 0.42),
        "critic": (0.92, 0.42),

        "guardrails": (0.82, 0.32),
        "clarify": (0.66, 0.22),
        "hitl": (0.82, 0.22),
        "await": (0.82, 0.14),
        "execute": (0.82, 0.06),
    ...
    online_edges: List[Edge] = [
        Edge("start", "load_session"),
        Edge("load_session", "validate_user"),
        Edge("validate_user", "load_ltm"),
        Edge("load_ltm", "classify"),
        ...
        Edge("simple_read", "format"),

        Edge("rag", "policy_reason"),
        Edge("policy_reason", "format"),

        Edge("draft_action", "data_reason", curve=0.10),
        Edge("draft_action", "critic", curve=-0.10),
        Edge("data_reason", "guardrails"),
        Edge("critic", "guardrails"),
        ...
        Edge("guardrails", "hitl", "need_HITL", "dashed", curve=-0.10),
        Edge("hitl", "await"),
        Edge("await", "execute", "approved", "dashed"),
        Edge("await", "format", "rejected", "dashed"),
    ...
    draw_flowchart(
        ...
        title="Online Decision Graph (LangGraph Runtime) — HR Decision Assistant",
        outpath=out_dir / "hr_decision_graph.png",
        figsize=(20,11),
        dpi=240
    )

    # ----------------------------
    # 2) OFFLINE POLICY INGESTION GRAPH
    # ----------------------------
    offline_nodes = {
        "start": "ingest_docs_start",
    ...
    offline_edges = [
        ("start", "extract", None, "solid"),
        ("extract", "chunk", None, "solid"),
        ("chunk", "embed", None, "solid"),
        ("embed", "upsert", None, "solid"),
        ("upsert", "manifest", None, "solid"),
        ("manifest", "end", None, "solid"),
    ]

    draw_flowchart(
        offline_nodes, offline_edges, offline_positions,
        title="Offline Policy Ingestion Graph — PDFs → Chunks → Embeddings → Vector DB",
        outpath=out_dir / "policy_ingestion_graph.png",
        figsize=(14,9),
        dpi=240
    )
    ...
    # ----------------------------
    draw_architecture(outpath=out_dir / "architecture.png")

    # ----------------------------
    # 4) ARCHITECTURE SPEC OVERVIEW (single image, easy to read)
    # ----------------------------
    draw_architecture_spec_overview(outpath=out_dir / "architecture_spec.png")


def draw_architecture_spec_overview(outpath: str = "architecture_spec.png"):
    """
    One clean multi-panel image: system overview + online flow + offline flow.
    Matches ARCHITECTURE.md / ARCHITECTURE_DIAGRAMS.md for easy reference.
    """
    ...
    # Simple boxes (x, y, w, h) in 0-1 coords
    ...
    def arrow(ax, start, end, label=None):
        ...
        if label:
            ...
            ax.text(mid[0], mid[1] + 0.02, label, ha="center", fontsize=7)
    # Layout: User -> CLI -> LangGraph; LangGraph -> LLM, MCP, Policies
    ...
    ax1.plot([0.14, 0.16], [0.52, 0.52], "k-", lw=1.2)
    ax1.annotate("", xy=(0.16, 0.52), xytext=(0.14, 0.52), arrowprops=dict(arrowstyle="->", lw=1.2))
    ax1.plot([0.30, 0.32], [0.52, 0.52], "k-", lw=1.2)
    ax1.annotate("", xy=(0.32, 0.52), xytext=(0.30, 0.52), arrowprops=dict(arrowstyle="->", lw=1.2))
    ax1.plot([0.54, 0.58], [0.65, 0.65], "k-", lw=1.2)
    ax1.annotate("", xy=(0.58, 0.65), xytext=(0.54, 0.65), arrowprops=dict(arrowstyle="->", lw=1.2))
    ax1.plot([0.54, 0.58], [0.38, 0.38], "k-", lw=1.2)
    ...
    # ---- Panel 2: Online decision flow (compact) ----
    ...
    nodes2 = [n_val, n_mem, n_cls, n_route, n_fetch, n_rag, n_write, n_fmt, n_upd]
    for x, y, label in nodes2:
    ...
    # ---- Panel 3: Offline ingestion ----
    ax3 = fig.add_subplot(3, 1, 3)
    ax3.set_axis_off()
    ax3.set_title("3. Offline policy ingestion (batch)", fontsize=12, pad=8)
    off_nodes = [
        (0.08, 0.5, "PDFs"),
        (0.24, 0.5, "extract_text"),
        (0.40, 0.5, "chunk"),
        (0.56, 0.5, "embed"),
        (0.72, 0.5, "upsert_vector_store"),
        (0.88, 0.5, "manifest"),
    ]
...
if __name__ == "__main__":
    # When run from script dir (e.g. architecture/), output there
